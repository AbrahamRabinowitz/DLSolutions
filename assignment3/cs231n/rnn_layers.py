...
def rnn_step_backward(dnext_h, cache):
    ...
    dout = dnext_h *(np.ones_like(next_h) - (next_h**2))
    dx = dout @ Wx.T
    dprev_h = dout @ Wh.T
    dWx = x.T @ dout 
    dWh  = prev_h.T @ dout 
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      if i == 0:
        h[:,i,:] = rnn_step_forward(x[:,i,:], h0,Wx,Wh,b)[0]
      else:
        h[:,i,:] = rnn_step_forward(x[:,i,:], h[:,i-1,:],Wx,Wh,b)[0]
...
def rnn_backward(dh, cache):
    ...
    for j in reversed(range(T)):
      if j == 0:
        cache = (x[:,j], h0, Wx, Wh, b, h[:,j])
      else:
        cache = (x[:,j], h[:,j-1], Wx, Wh, b, h[:,j])
      dx_j,dh0, dWx_j, dWh_j, db_j = rnn_step_backward(dh[:,j] + dh0, cache)
      dWh += dWh_j
      dx[:,j] = dx_j
      dWx += dWx_j
      db += db_j
