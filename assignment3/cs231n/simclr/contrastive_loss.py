...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_k_N = sim_exp(z_k,z_k_N)
        temp_k = 0
        temp_k_N = 0
        for i in range(N):
          if i!=k:
            temp_k += sim_exp(z_k,out[i]) + sim_exp(z_k,out[i + N])
            temp_k_N += sim_exp(z_k_N, out[i]) + sim_exp(z_k_N, out[i + N])
          else:
            temp_k += sim_exp(z_k,out[i + N])
            temp_k_N += sim_exp(z_k_N, out[i])
        total_loss -= (torch.log(sim_k_N/temp_k) +torch.log(sim_k_N/temp_k_N))
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(torch.mul((out_left/norm_left),(out_right/norm_right)), dim = 1, keepdim = True)
    pass
...
def compute_sim_matrix(out):
    ...
    norm_out = out/norms
    sim_matrix = torch.matmul(norm_out, torch.transpose(norm_out,0,1))
    pass
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    stacked_pos_pairs = torch.cat([pos_pairs, pos_pairs], dim = 0)
    pass
    ...
    numerator = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.exp(stacked_pos_pairs/tau)
    pass
