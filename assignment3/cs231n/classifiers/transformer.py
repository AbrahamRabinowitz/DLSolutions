...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions_encoded = self.positional_encoding(captions)

        video_embedded = self.visual_projection(features).unsqueeze(1)


        tgt_mask = torch.tril(torch.ones((T,T)))

        output = self.transformer(captions_encoded, video_embedded, tgt_mask)
